...
describe("Header component", () => {
  ...
  it("should call componentDidUpdate twice", () => {
    const componentDidUpdateSpy = jest.spyOn(
      Header.prototype, "componentDidUpdate"
    );

    const component = shallow(
      <Header onClickBody={true} showFunc={() => {}} />
    );
  ...
  it("should call linkTo function twice", () => {
    ...
    const component = shallow(
      <Header linkTo={linkToSpy} />
    );

    component
      .find(".header-nav WithStyles(ForwardRef(ListItemText))").first()
      .simulate("click");
    component
      .find(".header-nav WithStyles(ForwardRef(ListItemText))").last()
      .simulate("click");
