...
let chats, initialState, successState, failureState;
beforeEach(() => {
  chats = [
    {
      id: 1,
...
describe("ChatList component", () => {
  it("should rendered self", () => {
    ...
    expect(
      component.find("WithStyles(ForwardRef(ListItemText))").first().props().primary
    ).toBe("ChatsName1");
    expect(
      component.find("WithStyles(ForwardRef(ListItemText))").last().props().primary
    ).toBe("ChatsName2");
  ...
  it("should delete chat", () => {
    ...
    component
      .find("WithStyles(ForwardRef(Fab))").first()
  ...
  it("should add chat", () => {
    ...
    component
      .find("WithStyles(ForwardRef(TextField))").first()
      .simulate("change", { target: { name: "name", value: "New chat" } });
    component
      .find("WithStyles(ForwardRef(TextField))").last()
